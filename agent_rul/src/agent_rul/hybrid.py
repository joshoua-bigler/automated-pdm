...
def main(tracking: dict, config_template: str | Path):
  ...
  cfg0 = run_manager(tracking=tracking, data=raw, config_template=CONFIG_DIR/'gen'/config_template, out_dir=CONFIG_DIR / 'gen', use_cache=True)
  cfg = cfg0.get('config')
  if not cfg:
    raise ValueError('failed to create configuration')
  data = preprocessing(data=raw, config=cfg)
  search_space = build_search_space(cfg=cfg)
...
if __name__ == '__main__':
  ...
  tracking = {
      ...
      'version': '1.1.0',
      'dataset':{'name': 'cmapps', 'fd_number': 1}, #{'name': 'femto', 'downsampling': {'apply': False, 'factor': 1, 'group_by': 'unit'}}, # {'name': 'cmapps', 'fd_number': 1},
      'baseline_result': {}, # {'version': '0.9.2'}
      'num_samples': 2,
      'time_budget_s': 2 * 3600,
      'standalone': False,
  }
  tracking['experiment_name'] = create_experiment_name(tracking=tracking, approach=Approach.HYBRID)
  main(tracking=tracking, config_template='rul_pipeline_template.yml')
